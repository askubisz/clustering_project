...
aggregation_labels=aggregation.labels[0]
aggregation=aggregation.data

# Standardise data and add noise
scaled_aggregation=StandardScaler().fit_transform(aggregation)

noise=np.column_stack((np.random.uniform(min(scaled_aggregation[:,0]),max(scaled_aggregation[:,0]),200), np.random.uniform(min(scaled_aggregation[:,1]),max(scaled_aggregation[:,1]),200))) # Noise

scaled_aggregation=np.vstack((noise, scaled_aggregation))

aggregation_labels=np.append([-1]*200, aggregation_labels)
...
classix_radius=0.09
classix_minpts=6
bandwidth = estimate_bandwidth(scaled_aggregation, quantile=0.122)


# Fit models
kmeans=KMeans(n_clusters=n_clusters, init='k-means++', n_init=10).fit(scaled_aggregation)
minibatch_kmeans=MiniBatchKMeans(n_clusters=n_clusters, init='k-means++', n_init=10).fit(scaled_aggregation)
pam=KMedoids(n_clusters=n_clusters, init='k-medoids++').fit(scaled_aggregation)
dbscan=DBSCAN(min_samples=min_samples, eps=eps).fit(scaled_aggregation)
optics=OPTICS(min_samples=min_samples, metric='euclidean', cluster_method='dbscan', eps=eps).fit(scaled_aggregation)
hdbscan=HDBSCAN(min_samples=min_samples_hdbscan, min_cluster_size=min_cluster_size).fit(scaled_aggregation)
mean_shift=MeanShift(bandwidth=bandwidth, cluster_all=False).fit(scaled_aggregation)
single=AgglomerativeClustering(n_clusters=n_clusters, linkage='single').fit(scaled_aggregation)
complete=AgglomerativeClustering(n_clusters=n_clusters, linkage='complete').fit(scaled_aggregation)
ward=AgglomerativeClustering(n_clusters=n_clusters, linkage='ward').fit(scaled_aggregation)
spectral=SpectralClustering(n_clusters=n_clusters, affinity='nearest_neighbors', n_neighbors=n_neighbors, assign_labels='kmeans').fit(scaled_aggregation)
classix=CLASSIX(sorting='pca', group_merging='density', radius=classix_radius, minPts=classix_minpts, post_alloc=False, verbose=0).fit(scaled_aggregation)
denmune, _=DenMune(train_data=scaled_aggregation, k_nearest=denmune_neigh).fit_predict(show_plots=False, show_analyzer=False)
c, num_clust, req_c=FINCH(data=scaled_aggregation, req_clust=n_clusters, distance='euclidean', verbose=False)
...
ax1.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[kmeans.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax2.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[minibatch_kmeans.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax3.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[pam.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax4.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[dbscan.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax5.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[optics.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax6.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[hdbscan.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax7.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[mean_shift.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax8.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[single.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax9.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[complete.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax10.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[ward.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax11.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[spectral.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax12.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[classix.labels_], s=20)
...
colors = np.append(colors, ["#000000"])
ax13.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[denmune_labels], s=20)
...
colors = np.append(colors, ["#000000"])
ax14.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[req_c], s=20)
ax14.set_title("FINCH", size=20)
ax14.set_xticks(())
ax14.set_yticks(())

# true labels plot
colors = np.array(list(islice(cycle(colours_distinct), len(np.unique(aggregation_labels)))))
colors = np.append(colors, ["#000000"])
ax15.scatter(scaled_aggregation[:, 0], scaled_aggregation[:, 1], color=colors[aggregation_labels], s=20)
